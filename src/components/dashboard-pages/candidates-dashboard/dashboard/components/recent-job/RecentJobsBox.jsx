...
const RecentJobsBox = () => {
  ...
  // فیلتر جاب‌ها بر اساس دسته‌بندی کاربر لاگین شده
  const matchedJobs = sortedJobs?.filter(
    (job) =>
      job?.job_category_id === userAuth?.user?.jobseekerprofiles?.job_category_id
  )?.slice(0, 7);
  ...
  // ***** بقیه فانکشن ها مثل clearAll و handlers همون قبلی هستن و تغییر نمی‌کنن *****
